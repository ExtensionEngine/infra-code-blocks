...
| `dashboardConfig`<br/>`DashboardBuilder.Config` | Dashboard-level settings merged over the builder defaults before `build()` runs. Default: `{}`. |
...
| `dashboardConfig`<br/>`DashboardBuilder.Config` | Dashboard-level settings merged over the builder defaults before `build()` runs. Default: `{ refresh: '1m' }`. |
