...
type PersistentStorageVolume = { name: pulumi.Input<string>; };
...
export class ImprovedEcsService extends pulumi.ComponentResource {
  name: string;
  ...
  constructor(
    ...
    const taskExecutionRole = this.createTaskExecutionRole(argsWithDefaults.taskExecutionRoleInlinePolicies);
    const taskRole = this.createTaskRole(argsWithDefaults.taskRoleInlinePolicies)
  ...
  private createTaskDefinition(
    ...
    const containerDefinitions = containers.map(container => this.createContainerDefinition(container));
  ...
  private createContainerDefinition(container: EcsContainer) {
    return pulumi
      .all([
        container.name,
        container.image,
