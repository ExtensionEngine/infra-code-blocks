...
export class OriginFactory {
  ...
  getWebsiteBucket(
    id: string,
  ): [aws.s3.Bucket, aws.s3.BucketWebsiteConfigurationV2] {
  ...
  getLoadBalancer(
    ...
    const webServer = new studion.WebServerBuilder(name)
      ...
      .build({ parent: this.parent });

    return webServer.lb.lb;
  }

  private uploadAssets(name: string, bucket: aws.s3.Bucket) {
    const dir = path.join(
      process.cwd(),
